...
## A relative abundance plot using the data
```{r}
# relative abundance in toad samples
spe <- t(input_toad$data_loaded)
spe.relf <- 100*(spe/rowSums(spe))
...
spe.relf2 <- 100*(spe2/rowSums(spe2))

# clean up data
spe_rel_t <- t(spe.relf) %>%
  as.data.frame() %>%
  rownames_to_column(var = "otu.name")
names(spe_rel_t)[2] <- "rel_abund_toad" # name the second column something
spe_rel_t <- spe_rel_t[,c(1,2)] # only take the first two columns (out ID and relative abundace on toad)
spe_rel_e <- t(spe.relf2) %>%
  as.data.frame() %>%
  rownames_to_column(var = "otu.name")
names(spe_rel_e)[2] <- "rel_abund_env"
spe_rel_e <- spe_rel_e[,c(1,2)]
