...
```{r}
list_env <- c("Sediment","Soil","Water")
...
W_frame <- comparison_test_lifeenv$W.taxa %>%
  mutate(Enriched = case_when(W_stat < 40 ~ "Transient, not significant",
...
volcano.plot <- ggplot(W_frame) +
  ...
  geom_hline(mapping = aes(yintercept = 40), col = "red", lty = 2)
volcano.plot
# ggsave(paste0(work_dir,"Figs_Tables/volcano_plot_color.png"),
#        volcano.plot, dpi=400, width=7, height=5)
```

- toad only, comparing the life stages
```{r}
# load(file=paste0(work_dir,"02_hypothesisbasedanalsysis/ANCOM_lifestage.RData"))

# make a volcano plot showing the associated microbes
W_frame_ls <- comparison_test_lifestages$W.taxa %>%
  mutate(Enriched = case_when(W_stat < 20 ~ "Transient, not significant",
                              Environment_relAbund > Toad_relAbund ~ "Transient",
                              TRUE ~ "Symbiotic"))

yint = 26 - (26 - 18)/2 # want to put intercept between 18 and 26 because W=26+ is significant and 18 and less is not
# figure out order of ecol for legend
Ecol_list <- c("Symbiotic", "Transient", "Transient, not significant")
W_frame$Enriched <- factor(W_frame$Enriched, levels = Ecol_list)
# write.table(W_frame,
