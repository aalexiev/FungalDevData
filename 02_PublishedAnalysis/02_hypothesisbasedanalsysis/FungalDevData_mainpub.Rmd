...
2) ANCOM comparing life stages to each other, no environmental samples at all.
```{r}
# filter and prep files in ANCOM format
# Mapping file filtered
mf_temp <- map_ancom %>%
  filter(Life_Stage_Simplified %in% list_life)
# OTU file filtered
otu_temp <- input_toad$data_loaded[ , which(colnames(input_toad$data_loaded) %in% mf_temp[ , 1])]
# Transpose/ANCOM formatting
otu_temp_t <- as.data.frame(t(otu_temp)) %>%
  rownames_to_column("Sample.ID")

# run ANCOM, this will use a Kruskall Wallis test
comparison_test_lifestages <- ANCOM.main.myc(OTUdat = otu_temp_t,
                                      Vardat = mf_temp,
                                      adjusted = F,
                                      repeated = F,
...
# This tells you if your data has a lot of zeroes/skew, as most microbial data do.
# everything below the red line gets removed.
comparison_test_lifestages$PLot.zeroes
# This tells you which (colored) OTU's have a high effect size and significance
comparison_test_lifestages$Plot.volcano
comparison_test_lifestages$Plot.volcano.0.9
# this helps you determine if the critical w was chosen correctly. We want the valley between a bimodal distribution, in theory.
comparison_test_lifestages$Plot.critical.w
# I think W=20 is suggested.
# save(comparison_test_lifestages,
#      file = paste0(work_dir,"02_hypothesisbasedanalsysis/ANCOM_lifestage.RData"),
#      envir = .GlobalEnv)
# write.table(comparison_test_lifestages$W.taxa, paste0(work_dir,"02_hypothesisbasedanalsysis/ANCOMlifestage_taxabunds.txt"), sep="\t", quote=F)
```

```{r}
#still using a Wilcoxon test but "for" loop will go through all the variations of pairs

#make lists of env and life variables that will each pair with each other
#I want each one of env to pair with each one of life
list_env <- c("Sediment","Water")
list_life <- c("Eggs","Tadpole","Metamorph","Subadult","Adult")
...
W_frame <- comparison_test_lifeenv$W.taxa %>%
  mutate(Enriched = case_when(W_stat < 26 ~ "Transient, not significant",
                              env_relAbund > life_relAbund ~ "Transient",
                              TRUE ~ "Symbiotic"))

yint = 26-(26-18)/2 # want to put interept between 18 and 26 because W=26+ is significant and 18 and less is not
...
volcano.plot <- ggplot(W_frame) +
  ...
  geom_hline(mapping=aes(yintercept=yint), col="red", lty=2)
volcano.plot
...
merged_filt_ecol$life_relAbund_ln <- log(merged_filt_ecol$life_relAbund)
