...
# in excel, add column for whether they are transient or symbionts; read in that file
...
Make a percentile rank abundance based plot using the data, since ANCOM is actually using rank abundances to identify indicator OTU's
```{r}
# calculate relative then rank abundance then percentile for each OTU in the whole dataset
...
merged_spe$rank_abund_toad <- NA # add as new column for later use
...
merged_spe$rank_abund_env <- NA
merged_spe$rank_abund_env[order.abund_ENV] <- 1:nrow(merged_spe)

# convert rankings to percentile rankings for toad and env samples
ecdf_toad <- ecdf(merged_spe$rank_abund_toad) # produces a "function" call that will tell you percentile of any number in range merged_spe$rank_abund_toad
merged_spe$percentile_toad <- NA
merged_spe$percentile_toad <- ecdf_toad(merged_spe$rank_abund_toad) # will give you quantile of every OTU in toadOTU_rankings
ecdf_env <- ecdf(merged_spe$rank_abund_env)
merged_spe$percentile_env <- NA
merged_spe$percentile_env <- ecdf_env(merged_spe$rank_abund_env)
...
list_OTUs <- list("OTU_15", "OTU_166", "OTU_73", "OTU_1", "OTU_25",
                  ...
                  "OTU_34", "OTU_81", "OTU_14", "OTU_83", "OTU_166",
                  "OTU_9", "OTU_14")
merged_filt_spe <- merged_spe %>%
  dplyr::filter(otu.name %in% list_OTUs)

# save the above file and add an "ecol" column for whether something is transient or symbiotic (based on the abundance data from ANCOM)
# write.table(merged_filt_spe,
#             paste0(work_dir,"Figs_Tables/Symbionts_fig/merged_filt_spe.txt"),
#             quote=F)
```

```{r}
# read in new file with putative symbiotic data
merged_filt_ecol <- read.csv(paste0(work_dir,"Figs_Tables/Symbionts_fig/merged_filt_ecol.csv"))

# plot significant ANCOM data
# rank abundance plot of just the OTU's in toads
rank_toads <- ggplot(merged_filt_ecol) + 
  geom_point(aes(x=rank_abund_toad, y=rel_abund_toad, color=Ecol)) +
  xlab("rank abundance on toad samples") +
  ylab("relative abundance on toad samples")

# rank abundance plot of just the OTU's in env
rank_env <- ggplot(merged_filt_ecol) + 
  geom_point(aes(x=rank_abund_env, y=rel_abund_env, color=Ecol)) +
...
rank_plot <- ggplot(merged_filt_ecol) + 
  geom_point(aes(x=percentile_env, y=percentile_toad, color=Ecol)) +
  xlab("Percentile Rank Abundance on Environmental Samples") +
  ylab("Percentile Rank Abundance on Toad Samples") +
  labs(color = "Putative host relationship") +
  xlim(0,1) + ylim(0,1) +
...
# as per Melissa's input, relative abundance only might be good enough for this graph since that's what ANCOM uses
rel_plot <- ggplot(merged_filt_ecol) + 
  geom_point(aes(x=rel_abund_env, y=rel_abund_toad, color=Ecol)) +
...
# ggsave(paste0(work_dir,"Figs_Tables/Symbionts_fig/rank_dotplot.png"), rank_plot,
#        dpi=300, width=7, height=4)

# ggsave(paste0(work_dir,"Figs_Tables/Symbionts_fig/rel_dotplot.png"), rel_plot,
#        dpi=300, width=7, height=4)
```
